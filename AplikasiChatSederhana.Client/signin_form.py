...
class SignInForm(ft.UserControl):
    ...
    def btn_signin(self, e):
        if not self.text_user.value:  
            self.text_user.error_text="Email cannot be blank!"
    ...
    def build(self):
        ...
        self.text_user = ft.TextField(label="User Email")
        self.text_password = ft.TextField(
            label="Password", password=True, can_reveal_password=True
        )
        self.text_signin=ft.ElevatedButton(text="Sign in", bgcolor=ft.colors.CYAN_500, color=ft.colors.WHITE,width=150,height=50,on_click= self.btn_signin)
